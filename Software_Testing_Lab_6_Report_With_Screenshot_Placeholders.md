...
**Status convention:** `Not Executed` means this report writer did not perform the live action. Execute the listed steps, replace Actual Result with observed facts, and change status only then. â€œTo Be Verifiedâ€ is used where external systems are required.
...
| TC-05 System Invalid | Verification invalid/expired/reused code | Registered unverified account. Wrong, expired, or previously used six-digit code. | Submit code. | â€œInvalid or expired verification codeâ€ response; account remains unverified/inactive. | Not Executed. Backend searches only unused, unexpired matching records. Capture SS-04. |
...
| TC-24 Integration Invalid | Donation validation | Donation page. Missing category/name/email/phone or amount â‰¤0. | Submit invalid values. | Client/API rejects data and no valid payment initiation occurs. | Not Executed. Backend validates required donor fields, category and amount `numeric|min:1`. Capture SS-20. |
...
| TC-31 Integration Normal | Diin AI valid question with history | Laravel API and Python AI service are running. Valid Islamic question â‰¤1000 chars. | Open Diin AI; ask question; ask follow-up. | API passes message/history to AI service; response and any sources appear. | Not Executed. Capture SS-26 only with an actual response. |
...
|---|---|---|---|---|
| BUG-01 | `PUT /api/v1/about` is registered in the public `v1` group and invokes `AboutContentController@update`; a second protected admin route also updates it. Therefore an unauthenticated caller can potentially overwrite about-page content. | High | Openâ€”static inspection | Remove the public PUT route. Keep only the `auth:api` + `admin` protected route. Add authorization/feature tests. |
| BUG-02 | `routes/api.php` registers `GET /api/v1/ai/status` as `AIController@status`, but the inspected `AIController` contains `chat` and `chatWithHistory` only; no `status` method was found. Calling this route is expected to fail at runtime. | Medium | Openâ€”static inspection | Implement `status()` with a safe health response, or remove the route. Add a route-level test. |
| BUG-03 | Public activity details fetch the entire `/api/v1/activities` list and locate the ID client-side instead of requesting one detail record. This is inefficient and relies on the item remaining active/listed. | Low | Openâ€”design issue | Add a public `GET /activities/{id}` endpoint and fetch it directly; preserve 404 behaviour. |
| BUG-04 | The repository has multiple SQL dumps (`addin.sql`, `addiin.sql`, and `database/migrations/addiin.sql`) and Docker Compose references `./database/migration/addiin.sql` (singular `migration`), while the observed folder is `database/migrations`. Docker initialization may therefore miss the SQL file. | Medium | Openâ€”configuration inspection | Use one canonical schema/migration source; correct the Compose volume path; validate with clean-container startup. |
| BUG-05 | `AboutContentController@update` accepts all request data and writes JSON without field-level validation. | Medium | Openâ€”static inspection | Use explicit validation rules and an allowlist; reject unexpected keys/oversized fields. |
...
| SS-25 | TC-29 | User sends message and admin replies (or show a legitimate conversation view); capture messages and status, masking private text if needed. | Proves messaging integration. **Figure SS-25: Userâ€“Admin Messaging Conversation.** | After TC-29; Required; `SS-25_Messaging.png` |
...
Below every figure, add: â€œThe figure shows the observed result for [TC ID]. It provides visual evidence that [expected outcome] was [observed/not observed].â€ Replace bracketed text with the real outcome.

## 12. Screenshot Collection Checklist

| Order | ID | Action | Expected screen/result | Filename |
|---:|---|---|---|---|
| 1â€“4 | SS-01 to SS-04 | Register a disposable test account, verify it, then test invalid code. | Registration/verification states. | As listed above |
| 5â€“8 | SS-05 to SS-08 | Login; test invalid login; update profile; test password validation. | Auth/profile evidence. | As listed above |
| 9â€“14 | SS-09 to SS-14 | Capture prayer times, activities, missing activity, optional admin activity, events. | Public/admin content. | As listed above |
| 15â€“17 | SS-15 to SS-17 | Submit Milad, update status as admin, calculate Zakat. | Booking/status/calculation. | As listed above |
| 18â€“21 | SS-18 to SS-21 | Test donation initiation/real callback/invalid input/history. | Payment workflow evidence. | As listed above |
| 22â€“25 | SS-22 to SS-25 | Submit contact, admin reply, invalid contact, messaging. | Communication evidence. | As listed above |
| 26â€“30 | SS-26 to SS-30 | AI normal/error, admin dashboard, denied access, 404. | AI, roles, error pages. | As listed above |
...
## 15. Screenshot Placeholders for Manual Evidence

Screenshots are included as visual evidence of the tested functionalities. The placeholders below are reserved for inserting the actual screenshots captured during system testing.

> **Insertion note:** Each centered blank area below is intentionally empty. Insert only a real screenshot captured during the corresponding manual test. A placeholder is not evidence of a pass result; retain the status stated in the detailed test case until execution has occurred.

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-01]
...
This screenshot demonstrates the successful registration process and the verification prompt displayed to the user.

**Related Test Case:** TC-01 — Successful Registration

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-02]

<br><br><br><br>

</div>

**Figure 2: Invalid Registration**

This screenshot demonstrates the validation messages shown when registration input is invalid.

**Related Test Case:** TC-02 — Invalid Registration

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-03]

<br><br><br><br>

</div>

**Figure 3: Successful Email Verification**

This screenshot demonstrates confirmation that a valid verification code has activated the user account.

**Related Test Case:** TC-03 — Successful Email Verification

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-04]

<br><br><br><br>

</div>

**Figure 4: Invalid or Expired Verification Code**

This screenshot demonstrates the system response to an invalid, expired, or reused verification code.

**Related Test Case:** TC-04 — Invalid or Expired Verification Code

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-05]
...
This screenshot demonstrates that valid user credentials lead to the authenticated application state.

**Related Test Case:** TC-05 — Successful Login

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-06]
...
This screenshot demonstrates the error feedback produced for invalid login credentials.

**Related Test Case:** TC-06 — Invalid Login

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-07]
...
This screenshot demonstrates that an authenticated user can save and view updated profile information.

**Related Test Case:** TC-07 — Profile Update

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-08]

<br><br><br><br>

</div>

**Figure 8: Password Validation/Error**

This screenshot demonstrates validation or error feedback during password change.

**Related Test Case:** TC-08 — Password Validation/Error

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-09]

<br><br><br><br>

</div>

**Figure 9: Prayer Times Display**

This screenshot demonstrates the public display of configured prayer times.

**Related Test Case:** TC-09 — Prayer Times Display

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-10]

<br><br><br><br>

</div>

**Figure 10: Admin Prayer-Time Management**

This screenshot demonstrates an administrator managing prayer-time data.

**Related Test Case:** TC-10 — Admin Prayer-Time Management

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-11]

<br><br><br><br>

</div>

**Figure 11: Activity Details**

This screenshot demonstrates the activity-detail view opened from the public activities list.

**Related Test Case:** TC-11 — Activity Details

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-12]

<br><br><br><br>

</div>

**Figure 12: Activity Not Found/Error**

This screenshot demonstrates graceful handling when an activity cannot be found.

**Related Test Case:** TC-12 — Activity Not Found/Error

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-13]

<br><br><br><br>

</div>

**Figure 13: Admin Activity Creation**

This screenshot demonstrates successful creation of an activity by an administrator.

**Related Test Case:** TC-13 — Admin Activity Creation

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-14]

<br><br><br><br>

</div>

**Figure 14: Islamic Events Display**

This screenshot demonstrates the display of Islamic event information.

**Related Test Case:** TC-14 — Islamic Events Display

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-15]

<br><br><br><br>

</div>

**Figure 15: Milad Booking Submission**

This screenshot demonstrates submission of a Milad booking and its initial recorded state.

**Related Test Case:** TC-15 — Milad Booking Submission

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-16]

<br><br><br><br>

</div>

**Figure 16: Milad Booking Status Update**

This screenshot demonstrates an administrative update to a Milad booking status.

**Related Test Case:** TC-16 — Milad Booking Status Update

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-17]

<br><br><br><br>

</div>

**Figure 17: Zakat Calculation Result**

This screenshot demonstrates the Zakat calculator inputs and displayed result.

**Related Test Case:** TC-17 — Zakat Calculation Result

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-18]

<br><br><br><br>

</div>

**Figure 18: Donation Initiation**

This screenshot demonstrates valid donation details immediately before or during payment initiation.

**Related Test Case:** TC-18 — Donation Initiation

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-19]

<br><br><br><br>

</div>

**Figure 19: Donation Callback/Payment Result**

This screenshot demonstrates the actual payment callback state shown by the application.

**Related Test Case:** TC-19 — Donation Callback/Payment Result

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-20]

<br><br><br><br>

</div>

**Figure 20: Invalid Donation Input**

This screenshot demonstrates validation feedback for missing or invalid donation input.

**Related Test Case:** TC-20 — Invalid Donation Input

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-21]

<br><br><br><br>

</div>

**Figure 21: Donation History**

This screenshot demonstrates the authenticated user's donation-history display.

**Related Test Case:** TC-21 — Donation History

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-22]

<br><br><br><br>

</div>

**Figure 22: Contact Form Submission**

This screenshot demonstrates successful submission of a public contact message.

**Related Test Case:** TC-22 — Contact Form Submission

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-23]

<br><br><br><br>

</div>

**Figure 23: Admin Contact Reply**

This screenshot demonstrates an administrator reading or replying to a contact message.

**Related Test Case:** TC-23 — Admin Contact Reply

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-24]

<br><br><br><br>

</div>

**Figure 24: Invalid Contact Form**

This screenshot demonstrates validation feedback for invalid contact-form input.

**Related Test Case:** TC-24 — Invalid Contact Form

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-25]

<br><br><br><br>

</div>

**Figure 25: Messaging Feature**

This screenshot demonstrates an authenticated user–administrator conversation view.

**Related Test Case:** TC-25 — Messaging Feature

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-26]

<br><br><br><br>

</div>

**Figure 26: Diin AI Response**

This screenshot demonstrates a live response returned by the Diin AI feature.

**Related Test Case:** TC-26 — Diin AI Response

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-27]

<br><br><br><br>

</div>

**Figure 27: Diin AI Validation/Service Error**

This screenshot demonstrates safe validation or service-error handling in Diin AI.

**Related Test Case:** TC-27 — Diin AI Validation/Service Error

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-28]
...
This screenshot demonstrates the administrator dashboard interface.

**Related Test Case:** TC-28 — Admin Dashboard

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-29]

<br><br><br><br>

</div>

**Figure 29: Unauthorized Admin Access**

This screenshot demonstrates denial or redirection when a non-administrator attempts to access administrator functionality.

**Related Test Case:** TC-29 — Unauthorized Admin Access

<div align="center">

<br><br><br><br>

[Insert Screenshot Here  SS-30]

<br><br><br><br>

</div>

**Figure 30: 404 Page / Invalid Route**

This screenshot demonstrates the application response to an invalid route.

**Related Test Case:** TC-30 — 404 Page / Invalid Route
